...
export default function MainHeader() {
  ...
  return (
    <header
      className="absolute top-0 z-52 w-full bg-gradient-to-bl from-yellow-500/50 to-green-700/50"
    >
...
<Link href="/">
  ...
  />
        ...
        <nav className="hidden items-center lg:flex">
          <NavItem text="Get to know us" />
          <NavItem text="What do we offer" />
          <NavItem text="How to contact us" />
          <CTA />
        </nav>
      ...
      {open && (
        ...
        >
          <MobileNavItem text="Get to know us" />
          <MobileNavItem text="What do we offer" />
          <MobileNavItem text="How to contact us" />
...
function NavItem({ text }: { text: string }) {
  return (
    <a
      href="#"
      className="flex h-24 items-center px-16 text-xl font-bold text-white transition hover:bg-gradient-to-b from-gray-200/30 to-slate-500/30"
    >
      {text}
    </a>
  )
...
function MobileNavItem({ text }: { text: string }) {
  return (
    <a
      href="#"
      className="px-4 py-4 text-lg font-bold text-white transition hover:bg-gradient-to-b from-gray-200/30 to-slate-500/30"
    >
      {text}
    </a>
  )
...
function CTA({ full = false }: { full?: boolean }) {
  return (
    <a
      ...
      rounded hover:border-2 border-green-200
      ${full ? "h-14 w-full" : "h-full px-8 py-4"}`}
    >
      Book an Appointment
    </a>
  )
